...
P("Overall risk of bias (over the eight applicable items; Q9 = NA excluded): Low = at most one item rated No and both key items (Q7 measurement, Q8 analysis) rated Yes; High = three or more items rated No; Moderate otherwise—so a single No or Unclear on either key item gives at least a Moderate rating. Each study is appraised as the source of the estimates taken from it here rather than as a whole publication: the Q8 variance requirement is judged on the estimates extracted for this review, a variance counting as available when the source printed an interval or when one could be recovered from what the source reports. A study whose extracted estimates carry no interval therefore rates No on Q8 whether or not the source printed the ratio itself; that rating reflects incomplete reporting of uncertainty, not evidence that the estimate is biased. Of %d studies assessed, %d were Low, %d Moderate, and %d High; the Moderate ratings arise chiefly where an estimate was reported as a point value without a variance (Q8) or where race/ethnicity ascertainment was limited (Q7)."
