...
P("Main analysis: “yes (representative)” = the estimate carried into the main analysis as the group’s benchmark; “no (overlaps representative)” = an overlapping estimate for the same cell, used only when the representative is re-selected in the sensitivity analyses (Table 6); “no (AI/AN undercount)” = an unlinked national-registry AI/AN estimate demoted in favour of the IHS-linked representative; “no (registry-direct anchor)” = the SEER-Explorer reference value, not a screened study; “no (no usable IRR)” = the source named the subgroup but reported no rate or ratio from which an IRR could be formed.", True)
