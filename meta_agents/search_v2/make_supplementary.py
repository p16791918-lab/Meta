...
P("Method. A random sample of 200 excluded records (Python random.seed(42)) was read against the "
  "eligibility criteria. Because the search export carried an abstract for 99% of records, the entire "
  "excluded pool was then re-screened at the abstract level: each record's title and abstract were "
  "signature-scored for the eligibility profile — invasive female breast cancer, a U.S. "
  "population-based registry, an incidence rate or ratio, and a race-or-ethnicity comparison against "
  "White — with etiology, risk-factor, survival, staging-only, in-situ, treatment, cohort, "
  "international, and review designs down-weighted, and the abstracts of the highest-scoring "
  "candidates were read in full.", True)
...
P("Conclusion. The sample yielded about one eligible study in 200 and the abstract-level re-screen of "
  "the whole excluded pool two more; beyond these three it recovered no further eligible primary study "
  "— the one additional population-based race-versus-White breast-incidence report identified (Delta-"
  "region subtype incidence rate ratios) duplicated a region already represented and added no cell. "
  "The residual false-negative rate is thus small but non-zero, and single-reviewer screening with "
  "model assistance is recorded as a limitation in the Discussion. Separately, later source "
  "re-verification that rendered "
