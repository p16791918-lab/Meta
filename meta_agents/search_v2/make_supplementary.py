...
P("Main analysis: “yes (representative)” = the estimate carried into the main analysis as the group’s benchmark; “no (overlaps representative)” = an overlapping estimate for the same cell, used only when the representative is re-selected in the sensitivity analyses (Table 6); “no (AI/AN undercount)” = an unlinked national-registry AI/AN estimate demoted in favour of the IHS-linked representative; “no (registry-direct anchor)” = the SEER-Explorer reference value, not a screened study; “no (external comparator)” = an estimate that borrows an out-of-paper reference (a SEER-Explorer non-Hispanic White rate), retained only in the sensitivity analysis; “no (no usable IRR)” = the source named the subgroup but reported no rate or ratio from which an IRR could be formed.", True)
...
P("Table 6a. Low-risk-of-bias only (Moderate/High-RoB studies dropped): %d of %d cells unchanged, %d changed, %d dropped. The changed and dropped cells were concentrated in the disaggregated AANHPI subgroups (with a few age-specific and Middle Eastern cells), whose representatives are computed estimates from moderate-risk-of-bias sources; the aggregate and regional AI/AN representatives are unchanged because the IHS-linked estimates preferred in the main analysis are themselves low risk of bias."
