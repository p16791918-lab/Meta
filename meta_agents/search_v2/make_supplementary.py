...
# Counted from the rows just built, so the Note's arithmetic cannot drift from the
# table: a reader reconciling its row count with the estimate count in the text
# needs the anchor rows named.
_S4N = len(rows)
_S4REP = sum(1 for r in rows if r[-1].startswith("yes"))
_S4ANCH = sum(1 for r in rows if "anchor" in r[-1])
# Counting the unstratified-White rows here gives 31, not the 19 the main text
# reports, because this table lists overlaps as well as representatives. Say so.
_S4UW = sum(1 for r in rows if r[3] != disp_comparator("NHW"))
...
P("Overall risk of bias (over the eight applicable items; Q9 = NA excluded): Low = at most one item rated No and both key items (Q7 measurement, Q8 analysis) rated Yes; High = three or more items rated No; Moderate otherwise—so a single No or Unclear on either key item gives at least a Moderate rating. Each study is appraised as the source of the estimates taken from it here rather than as a whole publication, and the items divide accordingly: Q1–Q7 describe the study and its registry (sample frame, ascertainment, case numbers, description, coverage, case identification, and race/ethnicity measurement) and are judged at that level, while Q8 depends on the estimates themselves and is judged on those extracted for this review, a variance counting as available when the source printed an interval or when one could be recovered from what the source reports. A study whose extracted estimates carry no interval therefore rates No on Q8 whether or not the source printed the ratio itself; that rating reflects incomplete reporting of uncertainty, not evidence that the estimate is biased. Of %d studies assessed, %d were Low, %d Moderate, and %d High; the Moderate ratings arise chiefly where an estimate was reported as a point value without a variance (Q8) or where race/ethnicity ascertainment was limited (Q7)."
...
# record_id -> reader-facing author-year, so each row can name the study that filled
# the cell in the main analysis and the one that replaces it under the restriction.
_sens_ay = {}
for _r in rd("breast_extraction.csv"):
    _sens_ay.setdefault(_r["record_id"], _r.get("author_year", ""))


def _sens_src(rec):
    if not rec:
        return ""
    lab = _sens_ay.get(rec, "")
    return (" — " + cite(lab)) if lab else ""


def _sens_rows(rows):
    out = []
    for r in rows:
        main = r["main_irr"] + r.get("main_ci", "") + _sens_src(r.get("main_rec"))
        sens = ((r["sens_irr"] + r.get("sens_ci", "") + _sens_src(r.get("sens_rec")))
                if r["sens_irr"] else "not available under this restriction")
        out.append([disp_dim(r["dimension"]), disp_group(r["group"]), main, sens, r["status"]])
    return out


_SENS_HDR = ["Dimension", "Group", "Main analysis: IRR [95% CI] — study",
             "Under restriction: IRR [95% CI] — study", "Status"]
_SENS_W = [1400, 1800, 3400, 3400, 800]
...
TB(_SENS_HDR, _sens_rows(ch3), _SENS_W)
