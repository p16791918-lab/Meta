...
def main():
    ...
    box(ax, lex, 8.85, lew, 1.6,
        "Records excluded (n = 4,549):\nnot relevant to topic (n = 3,080);\n"
    ...
    box(ax, incx, 3.0, incw, 1.3,
        "Studies included in the review (n = 164)\n"
        "Quantitative synthesis: 54 studies   |   Narrative synthesis only: 110",
        fc="#cfe6d4")
    arrow(ax, lmx + lmw / 2, 5.4, lmx + lmw / 2, 4.3)

    box(ax, incx, 1.0, incw, 1.4,
        "Studies contributing ≥1 estimate to the quantitative synthesis (n = 54)\n"
        "202 extracted estimates → 83 representative estimates (one per analytic cell), from 24 studies;\n"
        "the remaining 30 studies contributed only overlapping estimates to the sensitivity analysis",
        fc="#cfe6d4", fs=7.0)
