...
const fs = require("fs");
const M = JSON.parse(fs.readFileSync("_maintext_manifest.json", "utf8"));
...
mainKids.push(new Paragraph({ spacing: { after: 200 },
  children: [new TextRun({ text: "Main Text — Tables and Figures", font: FONT, bold: true, size: 34 })] }));
mainKids.push(new Paragraph({ spacing: { after: 200 },
...
let inFigures = false;
for (const m of M) {
  if (m.type === "heading" && m.level === 1 && /^Figure \d/.test(m.text)) inFigures = true;
  const kids = inFigures ? figKids : mainKids;
  if (m.type === "heading") {
    // plain bold black heading (no Word Heading style: avoids the blue colour and
    // the style's left-hand bullet; keeps a clean journal look)
...
Packer.toBuffer(doc).then(b => { fs.writeFileSync("Main_Text_Tables_Figures.docx", b); console.log("wrote Main_Text_Tables_Figures.docx (" + (mainKids.length + figKids.length) + " blocks)"); });
