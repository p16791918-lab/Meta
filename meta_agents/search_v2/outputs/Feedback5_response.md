...
- **재발 방지 점검 추가**: 표와 그림이 다시 어긋나지 않도록 교차점검에 **[I] Table 1 vs 본문 그림** 항목을 신설 — Table 1의 38개 cell 각각에 대해 그림 데이터의 **값·구간·† 표기·‡ 표기 네 가지가 모두 일치**하는지 검사하며, 어느 하나라도 다르면 실패 처리. 현재 38/38 통과. 대조를 정확히 하기 위해 Table 1 출력에 analytic dimension 열을 추가(표의 섹션 제목은 표시용이라 AANHPI가 두 섹션으로 나뉘어 있었음). 전체 교차점검 A–I 전부 통과.
