...
A. Methods·Discussion·Supplementary Table 5는 모두 스크리닝·전문선정·추출·RoB가 **단일 리뷰어(저자)+LLM 보조**로 수행됐다고 정직하게 기술하고, 제2 독립 리뷰어 부재를 **한계로 명시**한다(3차 피드백 #4에 따라 2차의 "2인 독립+제3자" 문안을 철회). 저자가 실제로 제2 독립 리뷰어를 투입하면 문안과 리뷰어 간 일치도를 갱신해야 한다. PROSPERO 등록은 진행 예정이며 등록번호를 Methods에 삽입할 것이다. (→ 저자 확정 필요 사항)
...
- 단일 리뷰어+AI를 한계로 명시 유지(또는 제2 독립 리뷰어 실제 투입 시 문안·일치도 갱신) + PROSPERO 등록 (Q14) — 저자 조치.
