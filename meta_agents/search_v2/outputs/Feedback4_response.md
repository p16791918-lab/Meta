...
- **② LLM 모델, 역할, 재검토 표본, 누락 보고**: 제목/초록 선별은 large language model
 ...
 재분류했다.
 - **필터 완화(≥4 of 5) 재수행 — 신규 회수 0편**: 위 5-신호 AND 필터(183편)는 다섯 신호가 초록에 모두
   있어야 걸리므로, 적격 연구라도 발생률어(RATE)나 미국 맥락(US) 같은 신호 **하나를 초록이 아니라
   표에만** 실으면 놓친다(실제 회수된 3720은 발생률어가, 2609는 US 신호가 초록에 없어 AND-5로는 안
   걸렸다). 이 한-신호-누락을 체계적으로 점검하려 필터를 **5개 중 4개 신호**(유방암 필수 + 나머지 4개 중
   3개)로 완화했다. ≥4 집합은 5-신호(183편)를 포함하는 상위집합이며, **한 신호가 빠진 회수 기록들(3720·
   2609 등, 위 14편에 이미 편입)도 이 ≥4 필터가 포괄한다** — 즉 별도 감사가 아니라 이 완화 필터가 그
   회수를 설명한다. 이미 판정된 기록을 뺀 **미판정 4-신호 잔여 478편**을 전부 초록 전문으로 정독한 결과
   **새로 자격을 갖춘 미국 인구기반 인종별 유방암 발생률 연구는 0편**이었다 — 대다수가 사망/생존(SEER
   예후 nomogram), 비미국(라틴아메리카·이스라엘·캐나다 First Nations·브라질 등), 위험요인/유전, 검진접근,
   치료/이차암 SIR 연구였다. 주제상 근접한 소수(캘리포니아 AANHPI 이웃 SES별 발생률, 뉴멕시코 AI/
   Hispanic/NHW 추세, NPCR-SEER TNBC 주별 변이, SEER 연령×인종×수용체 발생률)는 전부 **학회 초록**
   (사전지정 문헌유형 제외 대상)이고 이미 포함된 연구와 같은 SEER/NPCR registry family를 근거로 해 대표·
   헤드라인에 신규 기여가 없었다. 4차 re-screen에서 보류로 표시했던 rec 1027(메인 주 AYA)도 전문 재확인
   결과 비히스패닉 백인 단일군을 전국값과 비교한 것으로 인종 간 비교가 없어 부적격으로 최종 판정.
 - **잔여 false-negative를 한계로 명시**: ≥4 완화로 한 신호가 초록에 빠진 표-전용 보고(3720·2609 유형)는
   회수되므로, 남는 사각지대는 **초록에 신호가 둘 이상 빠진(≤3 신호) 보고**로 좁혀진다. AND-5(183편)와
   ≥4(추가 478편) 두 관문을 모두 전문 정독했으나 ≤3-신호 표-전용 보고에 대한 잔여 false-negative는
   남으며, 이를 검토의 한계로 명시한다.
