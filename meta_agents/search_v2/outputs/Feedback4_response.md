...
- **② LLM 모델, 역할, 재검토 표본, 누락 보고**: 제목/초록 선별은 large language model
 ...
 재분류했다.
 - **필터 완화(≥4 of 5) 재수행 — 신규 회수 0편**: 위 5-신호 AND 필터(183편)는 초록에 다섯 신호가
   모두 있어야 걸리므로, 초록엔 인종별 발생률을 **표로만** 싣고 발생률어(RATE)나 미국 맥락(US)을 초록
   본문에 안 쓴 논문은 구조적으로 놓친다. 이 잔여 false-negative를 점검하려 필터를 **5개 중 4개 신호**로
   완화해 제외편에 다시 걸었고, 아직 판정 안 된 **478편**(정확히 4신호, 유방암 필수 + 나머지 4개 중 3개)이
   새로 걸렸다. 478편 전부를 초록 전문으로 정독한 결과 **새로 자격을 갖춘 미국 인구기반 인종별 유방암
   발생률 연구는 0편**이었다 — 대다수가 사망/생존(SEER 예후 nomogram), 비미국(라틴아메리카·이스라엘·
   캐나다 First Nations·브라질 등), 위험요인/유전, 검진접근, 치료/이차암 SIR 연구였다. 주제상 근접한
   소수(캘리포니아 AANHPI 이웃 SES별 발생률, 뉴멕시코 AI/Hispanic/NHW 추세, NPCR-SEER TNBC 주별
   변이, SEER 연령×인종×수용체 발생률)는 전부 **학회 초록**(사전지정 문헌유형 제외 대상)이고 이미 포함된
   연구와 같은 SEER/NPCR registry family를 근거로 해 대표·헤드라인에 신규 기여가 없었다. 4차 re-screen에서
   보류로 표시했던 rec 1027(메인 주 AYA)도 전문 재확인 결과 비히스패닉 백인 단일군을 전국값과 비교한
   것으로 인종 간 비교가 없어 부적격으로 최종 판정.
 - **초록 필터의 구조적 불완전성을 한계로 명시**: 위 결과로 초록 기반 키워드 필터가 완전하지 않음이
   확인된다 — 일부 적격 연구는 인종별 발생률을 초록이 아니라 표에만 실어(RATE 또는 US 신호가 초록에
   부재) 어떤 초록 수준 필터로도 완전 회수가 불가능하다. AND-5(183편)와 완화된 ≥4(478편) 두 관문 모두
   전문 정독했으나, 그 밖의 초록 미기재 표-전용 보고에 대한 잔여 false-negative는 남으며, 이를 검토의
   한계로 명시한다.
