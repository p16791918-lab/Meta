...
I thank the editor and reviewers for their careful reading. Below I respond to
...
**Response.** I acknowledge this and have not concealed it. Prospective
...
timeline. I have instead addressed the underlying goal of registration —
...
reproduces the entire analysis end-to-end from a clean environment. I describe
these openly available materials in the Methods as an important reproducibility
safeguard notwithstanding the absence of prospective registration (I have
softened the earlier wording so as not to imply that public code supersedes
registration). I am happy to add the completed-protocol document as a
...
> code are openly available (see Data availability). The openly available
> protocol, data, and code provide an important reproducibility safeguard
> notwithstanding the absence of prospective registration."
...
**Response.** This is the central methodological point of the paper, and I have
sharpened it rather than defended a summary rate. I do **not** offer the
...
*quantifies and exposes* the between-stratum variation that is my thesis: the
...
subtype, and age), not statistical noise. Accordingly I frame the aggregate
estimates as descriptive and place the inferential weight on the disaggregated
strata. I have made this framing explicit in the Methods and Limitations.
...
If the reviewer would nonetheless prefer, I can move the aggregate forest plot
(Fig 1) and the pooled point estimates to the Supplement and lead the Results
with the disaggregated analyses; I have retained them in the main text only
...
**Response.** Agreed. I have softened all mechanistic language to conditional
...
**Response.** I have replaced the absolute phrasing with an interpretability
...
**Response.** I have expanded the Methods and Declarations to specify the scope
of AI use and the verification procedure, and I describe its limits candidly. AI
...
primary sources. I do not claim a full independent double-extraction of every
...
I have also added the single-reviewer/AI-assisted extraction design to the
Limitations. (Note: an earlier draft overstated this as "100% human
verification"; I corrected it to reflect what was actually done.)
...
**Response.** I agree this is a deliberate specificity-over-sensitivity design,
and I have now justified it actively in the Methods (not only acknowledged it in
...
the major sources of national and regional incidence data are captured; I
nonetheless state plainly that a more sensitive, abstract-wide search could add
further regional analyses. If the editor wishes, I can run and report a
supplementary abstract-wide search to quantify any incremental yield.

> *Limitations:* "… my search was deliberately specific — anchoring the
...
> disparities only as a secondary result … but I cannot exclude that a more
...
**Response.** I used DerSimonian–Laird as the conventional random-effects
default and have added a justification. Critically, my inference does not depend
on the precise width of the aggregate confidence intervals — I emphasize the
*direction* of effects and the disaggregated strata — so the choice of variance
estimator has negligible bearing on the conclusions. I additionally confirmed
...
> conventional random-effects default; because my inference rests on the
...
- **Reference 27 (Gathani et al., 2026, England).** This is a recent
  ...
  comparison. I will re-verify the final citation details against the journal
  record at proof.
...
- **Publication bias.** Formal funnel/Egger tests were not applicable (<10
  ...
  uninterpretable). I added a qualitative assessment: registry data ascertain
...
I believe these revisions address the reviewers' concerns while preserving the
paper's central contribution. I thank the reviewers again for improving the
manuscript.
